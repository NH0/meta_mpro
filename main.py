...
class Instance:
    ...
    def _distance_ind(self,i,j):
        ...
        return distance(self._data[i],self._data[j])
...
def find_closest_target(list_targets, barycenter):
    smallest_distance_to_barycenter = distance(list_targets[0][1], barycenter)
    closest_target = list_targets[0]
    for target in list_targets[1:]:
        distance_to_barycenter = distance(target[1], barycenter)
...
def find_sensors(solution, list_targets):
    return []
